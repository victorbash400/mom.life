...
The family simulator is an explicit alternative connection for Apple Health, Fitbit, Withings, WhatsApp, and Instacart. It is stored separately from live provider validation, can be disconnected without removing the installed plugin, and is always labelled as simulated. Simulated WhatsApp messages enter the same provider-event and Intake Agent route as signed webhook deliveries. Simulated health values use the normalized Apple Health sample store, while provider-specific assignment tools read those values through the normal plugin runtime. Adding or removing a child immediately adds or removes that simulated identity and its health data. No simulator connection calls an external provider.
