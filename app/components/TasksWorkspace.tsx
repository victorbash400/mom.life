...
export function TasksWorkspace({ initialChild = "all", onClose }: { initialChild?: string; onClose: () => void }) {
  const { createTask, deleteTask, error, loaded, setTaskStatus, tasks } = useFamilyTasks(); const [status, setStatus] = useState<TaskFilter>("all"); const [child, setChild] = useState(initialChild); const [sort, setSort] = useState("newest"); const [creating, setCreating] = useState(false);
