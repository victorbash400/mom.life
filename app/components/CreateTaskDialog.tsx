...
export function CreateTaskDialog({ initialChild = "all", open, onCancel, onSubmit }: { initialChild?: string; open: boolean; onCancel: () => void; onSubmit: (child: string, text: string) => Promise<void> }) {
  ...
  useEffect(() => { const dialog = ref.current; if (!dialog) return; if (open && !dialog.open) { setChild(initialChild); setText(""); setError(undefined); dialog.showModal(); } else if (!open && dialog.open) dialog.close(); }, [initialChild, open]);
