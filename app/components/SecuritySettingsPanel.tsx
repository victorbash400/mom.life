...
export function SecuritySettingsPanel({ settings, onSave }: { settings: SecuritySettings; onSave: (settings: Pick<SecuritySettings, "enabled" | "alert_level" | "instructions">) => Promise<void> }) {
  ...
  return <section className={styles.settings}><div className={styles.group}><SettingsRow control={<SettingsSwitch checked={enabled} label="Safety monitoring" onChange={setEnabled} />} title="Monitoring" /><SettingsRow control={<select aria-label="Safety alert level" disabled={!enabled || busy} onChange={(event) => setAlertLevel(event.target.value as SecurityAlertLevel)} value={alertLevel}><option value="urgent">Urgent only</option><option value="important">Important</option><option value="all">All credible concerns</option></select>} title="Alert level" /><label className={styles.instructions}><span>Instructions</span><textarea disabled={!enabled || busy} maxLength={4000} onChange={(event) => setInstructions(event.target.value)} placeholder="What should the Safety Agent alert you about?" rows={5} value={instructions} /></label></div><footer>{error ? <small role="alert">{error}</small> : saved ? <small>Saved</small> : <span />}<button disabled={busy} onClick={() => void save()} type="button">{busy ? "Saving…" : "Save"}</button></footer></section>;
}
