...
export function SimulatorWhatsApp({ simulator }: { simulator: Simulator }) {
  ...
  useEffect(() => { if (messageList.current) messageList.current.scrollTop = messageList.current.scrollHeight; }, [state?.messages.length]);
  async function send() {
