...
export function SecurityWorkspace({ security, onClose }: { security: SecurityState; onClose: () => void }) {
  ...
  return <section className={styles.workspace}><header><WorkspaceHeader title="Security" onClose={onClose} /><nav aria-label="Security section"><button aria-pressed={view === "alerts"} onClick={() => setView("alerts")} type="button">Alerts</button><button aria-pressed={view === "settings"} onClick={() => setView("settings")} type="button">Settings</button></nav></header>{security.error ? <p className={styles.error} role="alert">{security.error}</p> : null}{security.loaded ? view === "alerts" ? <section className={styles.panel}><header><strong>Security alerts</strong><small>{active.length} active</small></header><SecurityDashboard onDismiss={security.dismiss} onRetry={security.retry} onSelect={setSelectedId} reviews={reviews} selectedId={selectedId} /></section> : security.settings ? <SecuritySettingsPanel onSave={security.saveSettings} settings={security.settings} /> : null : null}</section>;
}
