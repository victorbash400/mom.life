...
export function SecurityAlertList({ reviews, selectedId, onSelect }: { reviews: SecurityReview[]; selectedId?: string; onSelect: (id: string) => void }) {
  return <section aria-label="Safety alerts" className={styles.list}>{reviews.map((review) => <SecurityAlertRow key={review.id} onSelect={() => onSelect(review.id)} review={review} selected={review.id === selectedId} />)}</section>;
}
