...
export function ParentProfileWorkspace({ onBack, onInformation, onSettings, onTasks }: { onBack: () => void; onInformation: () => void; onSettings: () => void; onTasks: () => void }) {
  return <section className={styles.profile}><WorkspaceHeader title="" onClose={onBack} /><h1>Sarah</h1><div className={styles.cards}><button onClick={onTasks} type="button"><ClipboardCheck /><strong>Tasks</strong></button><button onClick={onInformation} type="button"><PinkFolderIcon /><strong>Information</strong></button><button onClick={onSettings} type="button"><Settings /><strong>Settings</strong></button></div></section>;
}
