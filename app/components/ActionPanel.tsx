...
type Props = { child?: ChildProfile; connections: Connections; mode: PanelMode; parentSettingsView: ParentSettingsView; security: SecurityState; simulator: SimulatorState; onModeChange: (mode: PanelMode) => void; onReturnHome: () => void };

export function ActionPanel({ child, connections, mode, parentSettingsView, security, simulator, onModeChange, onReturnHome }: Props) {
  const { family: { parent } } = useFamily();
  const closeTasks = child ? () => onModeChange("child") : () => onModeChange("home");
  return <section className={styles.panel} data-mode={mode}><OrganicPanelSurface mode={mode} />{mode === "home" ? <HomeActions connections={connections} onModeChange={onModeChange} /> : null}{mode === "ask" ? <AskWorkspace onClose={() => onModeChange("home")} /> : null}{mode === "calendar" ? <CalendarWorkspace onClose={() => onModeChange("home")} /> : null}{mode === "incoming" ? <IntakeWorkspace onClose={() => onModeChange("home")} /> : null}{mode === "education" ? <EducationWorkspace child={child} onClose={() => onModeChange("home")} /> : null}{mode === "security" ? <SecurityWorkspace onClose={() => onModeChange("home")} security={security} /> : null}{mode === "simulator" ? <SimulatorWorkspace onClose={() => onModeChange("home")} simulator={simulator} /> : null}{mode === "tasks" ? <TasksWorkspace initialChild={child?.id} onClose={closeTasks} /> : null}{mode === "parent-tasks" ? <TasksWorkspace onClose={() => onModeChange("parent")} /> : null}{mode === "plugins" ? <PluginStore connections={connections} onBack={() => onModeChange("home")} /> : null}{mode === "child" && child ? <ChildProfileWorkspace child={child} onBack={onReturnHome} onInformation={() => onModeChange("child-info")} onTasks={() => onModeChange("tasks")} /> : null}{mode === "child-info" && child ? <ChildInformationWorkspace child profile={child} onBack={() => onModeChange("child")} /> : null}{mode === "parent" ? <ParentProfileWorkspace onBack={onReturnHome} onInformation={() => onModeChange("parent-info")} onSettings={() => onModeChange("parent-settings")} onTasks={() => onModeChange("parent-tasks")} /> : null}{mode === "parent-info" ? <ChildInformationWorkspace profile={parent} onBack={() => onModeChange("parent")} /> : null}{mode === "parent-settings" ? <ParentSettingsWorkspace initialView={parentSettingsView} key={parentSettingsView} onBack={() => onModeChange("parent")} /> : null}</section>;
}
