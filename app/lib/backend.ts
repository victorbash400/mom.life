import { currentSession, sessionHeaders } from "./session";
...
export async function backend(path: string, init: RequestInit = {}) {
  try {
    ...
    if (!auth || !session) return Response.json({ error: "Sign in to continue." }, { status: 401 });
    const response = await fetch(`${backendUrl}/api/${path}${path.includes("?") ? "&" : "?"}family_id=${encodeURIComponent(session.family_id)}`, { cache: "no-store", ...init, headers: { ...Object.fromEntries(new Headers(init.headers)), ...auth } });
    if (!response.ok) {
