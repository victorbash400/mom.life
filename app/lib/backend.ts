import { currentSession, rememberSessionFamily, sessionFamily, sessionHeaders } from "./session";
const backendUrl = process.env.MOM_LIFE_BACKEND_URL ?? "http://127.0.0.1:8000";
export async function backend(path: string, init: RequestInit = {}, knownFamily?: string) {
  try {
    const auth = await sessionHeaders();
    if (!auth) return Response.json({ error: "Sign in to continue." }, { status: 401 });
    let familyId = knownFamily ?? await sessionFamily();
    if (!familyId) {
      const session = await currentSession();
      if (!session) return Response.json({ error: "Sign in to continue." }, { status: 401 });
      familyId = session.family_id;
      await rememberSessionFamily(familyId);
    }
    const response = await fetch(`${backendUrl}/api/${path}${path.includes("?") ? "&" : "?"}family_id=${encodeURIComponent(familyId)}`, { cache: "no-store", ...init, headers: { ...Object.fromEntries(new Headers(init.headers)), ...auth } });
    if (!response.ok) {
