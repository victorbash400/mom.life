...
export function useToolConnections() {
  ...
  async function connect(id: string) { await mutate(`/api/plugins/${id}?family_id=${familyId}`, "POST"); await refresh(); }
  ...
  async function disconnect(id: string) { await mutate(`/api/plugins/${id}?family_id=${familyId}`, "DELETE"); await refresh(); }
