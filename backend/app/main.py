...
from .schemas import ChatRequest, HealthResponse, ModelResponse, RuntimeResponse, TaskCreate, TaskUpdate
...
@app.get("/api/tasks")
def list_tasks(family_id: str) -> list[dict[str, str]]:
    return task_store.list(family_id)


@app.post("/api/tasks", status_code=201)
def create_task(body: TaskCreate) -> dict[str, str]:
    return task_store.create(body.family_id, body.child_id, body.text)


@app.patch("/api/tasks/{task_id}")
def update_task(task_id: str, body: TaskUpdate) -> dict[str, str]:
