...
Preserve the requested action boundary: preparing or drafting content does not authorize sending, publishing, purchasing, or changing an external record. Include an external action only when the user requested it. This also applies to follow-up actions after a reply or an automation trigger.
...
For assignments that create external records, require the worker to inspect the provider for an existing matching record first and reuse or update it when possible. Preserve explicit text-only constraints in every document-generation instruction.
